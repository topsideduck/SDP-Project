...
void FEHImage::Open(const char *filename)
{
	...
	{
		std::cout << CONSOLE_ERR(
			"File [" << CONSOLE_BLUE(filename) << "] is not a valid image file! Please use a "<< CONSOLE_GREEN(".png")<<
			" file\n");
		return;
	}
	...
	{
		std::cout << CONSOLE_ERR(
			"Image [" << CONSOLE_BLUE(filename) << "] is too large! Please use an image smaller than " << CONSOLE_GREEN(
				LCD_WIDTH) << "x" << CONSOLE_GREEN(LCD_HEIGHT) << "\n");
	}
}
